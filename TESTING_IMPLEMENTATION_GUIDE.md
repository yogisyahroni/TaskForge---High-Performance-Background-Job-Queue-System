# Panduan Implementasi Testing (Unit, Integration, dan E2E) untuk TaskForge

## 1. Gambaran Umum

Dokumen ini menyediakan panduan lengkap untuk mengimplementasikan sistem testing komprehensif dalam aplikasi TaskForge. Sistem testing mencakup unit testing, integration testing, dan end-to-end (E2E) testing untuk memastikan kualitas, keandalan, dan kinerja aplikasi.
...
```rust
// File: tests/unit/job_model_tests.rs
...
#[tokio::test]
async fn test_job_can_be_executed_pending() {
    let job = Job::new(
        Uuid::new_v4(),
        "test_job".to_string(),
        serde_json::json!({}),
...
#[tokio::test]
async fn test_job_can_be_executed_scheduled_future() {
    let mut job = Job::new(
        Uuid::new_v4(),
        "test_job".to_string(),
        serde_json::json!({}),
...
#[tokio::test]
async fn test_job_can_be_executed_scheduled_past() {
    let mut job = Job::new(
        Uuid::new_v4(),
        "test_job".to_string(),
        serde_json::json!({}),
...
use taskforge::{
    ...
    models::{Job, JobQueue, Worker, JobStatus},
};
use uuid::Uuid;
...
struct MockDatabase {
    ...
    queues: std::collections::HashMap<Uuid, JobQueue>,
    workers: std::collections::HashMap<Uuid, Worker>,
}
...
impl MockDatabase {
    ...
    async fn create_job(&mut self, job: Job) -> Result<Job, SqlxError> {
        self.jobs.insert(job.id, job.clone());
        Ok(job)
    }
    
    async fn get_job_by_id(&self, id: Uuid) -> Result<Option<Job>, SqlxError> {
        Ok(self.jobs.get(&id).cloned())
    }
    
    async fn update_job(&mut self, job: Job) -> Result<Job, SqlxError> {
        self.jobs.insert(job.id, job.clone());
        Ok(job)
    }
    
    async fn get_jobs_by_queue(&self, queue_id: Uuid) -> Result<Vec<Job>, SqlxError> {
        Ok(self.jobs.values()
...
#[tokio::test]
#[serial]
async fn test_job_service_create_job() {
    let mut mock_db = MockDatabase::new();
    let queue_service = Arc::new(QueueService::new(mock_db.clone()));
    let job_service = JobService::new(mock_db.clone(), queue_service);
    ...
    let created_queue = mock_db.create_job_queue(queue).await.unwrap();
    ...
    assert_eq!(created_job.job_type, "test_job");
    assert_eq!(created_job.status, JobStatus::Pending);
    assert_eq!(created_job.priority, 5);
}

#[tokio::test]
#[serial]
async fn test_job_service_get_job_by_id() {
    let mut mock_db = MockDatabase::new();
    let queue_service = Arc::new(QueueService::new(mock_db.clone()));
    let job_service = JobService::new(mock_db.clone(), queue_service);
    ...
    let created_job = mock_db.create_job(job).await.unwrap();
    
    // Ambil job menggunakan service
    let retrieved_job = job_service.get_job_by_id(created_job.id).await.unwrap();
    
    assert!(retrieved_job.is_some());
    let retrieved_job = retrieved_job.unwrap();
    assert_eq!(retrieved_job.id, created_job.id);
    assert_eq!(retrieved_job.job_type, "test_job");
}

#[tokio::test]
#[serial]
async fn test_job_service_update_job_status() {
    let mut mock_db = MockDatabase::new();
    let queue_service = Arc::new(QueueService::new(mock_db.clone()));
    let job_service = JobService::new(mock_db.clone(), queue_service);
    ...
    // Update status job
    let updated_job = job_service
        .update_job_status(created_job.id, JobStatus::Processing)
        .await
        .unwrap();
...
#[tokio::test]
#[serial]
async fn test_job_service_get_jobs_by_queue() {
    let mut mock_db = MockDatabase::new();
    let queue_service = Arc::new(QueueService::new(mock_db.clone()));
    let job_service = JobService::new(mock_db.clone(), queue_service);
    ...
    // Buat beberapa job untuk queue yang sama
    let job1 = Job::new(
        created_queue.id,
    ...
    let created_job1 = mock_db.create_job(job1).await.unwrap();
    let created_job2 = mock_db.create_job(job2).await.unwrap();
    
    // Ambil job berdasarkan queue
    let jobs = job_service
        .get_jobs_by_queue(created_queue.id)
        .await
        .unwrap();
    
    assert_eq!(jobs.len(), 2);
    assert!(jobs.iter().any(|j| j.id == created_job1.id));
    assert!(jobs.iter().any(|j| j.id == created_job2.id));
    
    // Verifikasi prioritas job dipertahankan
    let job1_in_result = jobs.iter().find(|j| j.id == created_job1.id).unwrap();
    let job2_in_result = jobs.iter().find(|j| j.id == created_job2.id).unwrap();
    
    assert_eq!(job1_in_result.priority, 5);
    assert_eq!(job2_in_result.priority, 7);
}

// Test untuk QueueService
#[tokio::test]
#[serial]
async fn test_queue_service_create_queue() {
    let mock_db = MockDatabase::new();
    let queue_service = QueueService::new(mock_db);
    ...
    assert_eq!(created_queue.name, "test_queue");
    assert_eq!(created_queue.priority, 5);
    assert!(created_queue.is_active);
}

#[tokio::test]
#[serial]
async fn test_queue_service_get_queue_by_id() {
    let mut mock_db = MockDatabase::new();
    let queue_service = QueueService::new(mock_db.clone());
    ...
    let retrieved_queue = queue_service
        .get_queue_by_id(created_queue.id)
        .await
        .unwrap();
    
    assert!(retrieved_queue.is_some());
    let retrieved_queue = retrieved_queue.unwrap();
    assert_eq!(retrieved_queue.id, created_queue.id);
    assert_eq!(retrieved_queue.name, "test_queue");
}

// Test untuk WorkerService
#[tokio::test]
#[serial]
async fn test_worker_service_register_worker() {
    let mock_db = MockDatabase::new();
    let worker_service = WorkerService::new(mock_db);
    ...
    let created_worker = worker_service.register_worker(worker).await.unwrap();
    
    assert_eq!(created_worker.name, "test_worker");
    assert_eq!(created_worker.worker_type, crate::models::worker::WorkerType::General);
    assert_eq!(created_worker.max_concurrent_jobs, 5);
    assert_eq!(created_worker.status, crate::models::worker::WorkerStatus::Offline); // Status default
}

// Test untuk fungsi validasi
#[tokio::test]
async fn test_job_service_validate_job() {
    let mock_db = MockDatabase::new();
    let queue_service = Arc::new(QueueService::new(mock_db.clone()));
    let job_service = JobService::new(mock_db, queue_service);
    ...
    assert!(job_service.validate_job(&valid_job).await.is_ok());
    
    // Test invalid job (priority out of range)
    ...
    assert!(job_service.validate_job(&invalid_job).await.is_err());
}
```
...
async fn test_job_crud_operations() {
    ...
    let queue_service = Arc::new(QueueService::new(db.clone()));
    let job_service = Arc::new(JobService::new(db.clone(), queue_service.clone()));
...
// Integration test untuk sistem autentikasi dan otorisasi
#[tokio::test]
#[serial]
async fn test_authentication_authorization_integration() {
    use taskforge::services::authentication_service::AuthenticationService;
    use taskforge::services::authorization_service::AuthorizationService;
    ...
    let authz_service = AuthorizationService::new(db.clone(), auth_service.clone());
    
    // Buat organisasi dan proyek untuk testing
    ...
    let created_org = db.create_organization(organization).await.unwrap();
    
    // Dalam implementasi nyata, kita juga akan membuat user dan API key untuk testing
    // Tapi untuk sekarang, kita hanya menguji bahwa service bisa diinisialisasi
    assert!(authz_service.check_permission("test_user", "job:read", created_org.id).await.is_ok());
}

// Integration test untuk sistem penjadwalan
#[tokio::test]
#[serial]
async fn test_scheduled_job_integration() {
    let pool = setup_test_db().await;
    let db = Database::new(pool);
    let queue_service = Arc::new(QueueService::new(db.clone()));
    let job_service = Arc::new(JobService::new(db.clone(), queue_service.clone()));
    ...
    let queue = JobQueue::new(
        Uuid::new_v4(), // project_id
        "scheduled_test_queue".to_string(),
        Some("Queue for scheduled job testing".to_string()),
        5,
        serde_json::json!({
            "max_retries": 3,
            "timeout_seconds": 300
    ...
    let future_time = Utc::now() + chrono::Duration::seconds(10); // 10 seconds in the future
    
    let scheduled_job = Job::new_scheduled(
        created_queue.id,
        "scheduled_test_job".to_string(),
    ...
    let created_job = job_service.create_job(scheduled_job).await.unwrap();
    
    assert_eq!(created_job.status, JobStatus::Scheduled);
    assert!(created_job.scheduled_for.is_some());
    
    // Verify the scheduled time is in the future
    let scheduled_time = created_job.scheduled_for.unwrap();
...
proptest! {
    ...
    fn test_valid_job_priority_always_validates(priority in 0i32..=9) {
        let job = Job::new(
            ...
            priority,
            Some(3),
            uuid::Uuid::new_v4(), // organization_id
        );
    ...
    c.bench_function("create_job", |b| {
        b.iter(|| {
            let job = Job::new(
                ...
                black_box(5),
                black_box(Some(3)),
                black_box(Uuid::new_v4()), // organization_id
            );
            black_box(job)
        })
    });
